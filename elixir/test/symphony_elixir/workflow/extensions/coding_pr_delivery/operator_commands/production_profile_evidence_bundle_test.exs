defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.OperatorCommands.ProductionProfileEvidenceBundleTest do
  use ExUnit.Case, async: true

  alias SymphonyElixir.Workflow.Extensions.CodingPrDelivery
  alias SymphonyElixir.Workflow.Extensions.CodingPrDelivery.OperatorCommands.ProductionProfileEvidenceBundle
  ...
  test "is registered as a Coding PR Delivery operator command" do
    assert ProductionProfileEvidenceBundle in CodingPrDelivery.operator_commands()
  end

  test "exports a bounded blocked bundle for the default Phase 2 plan" do
    assert {stdout, "", 0} = ProductionProfileEvidenceBundle.evaluate(["--plan", "linear_cnb_shadow", "--json"])

    payload = Jason.decode!(stdout)

    assert payload["schema"] == "coding_pr_delivery.production_evidence_bundle.v1"
    assert payload["status"] == "blocked"
    assert payload["phase4_ready"] == false
    assert payload["preflight"]["status"] == "missing"
    assert payload["evidence_packets"]["provided_count"] == 0
  ...
  test "merges request, preflight, and evidence packet files without reading referenced evidence" do
    ...
    assert {stdout, "", 0} =
             ProductionProfileEvidenceBundle.evaluate(
               [
                 "--phase2-plan-file",
                 "phase2.json",
    ...
    assert payload["evidence_request"]["provided"] == true
    assert payload["preflight"]["status"] == "passed"
    assert payload["evidence_packets"]["valid_count"] == 1
  ...
  test "does not echo unreadable file paths or raw args" do
    deps = %{
      ...
      production_evidence_bundle: fn _input, _opts -> flunk("bundle should not run") end
    }

    assert {"", stderr, 64} =
             ProductionProfileEvidenceBundle.evaluate(["--phase2-plan-file", "secret-phase2.json"], deps: deps)

    assert stderr =~ "Phase 2 plan file could not be read or parsed."
    refute stderr =~ "secret-phase2"

    assert {"", argv_stderr, 64} =
             ProductionProfileEvidenceBundle.evaluate(["--plan", :secret_plan], deps: valid_deps())
  ...
  test "rejects invalid deps without leaking raw deps" do
    assert {"", stderr, 70} = ProductionProfileEvidenceBundle.evaluate([], deps: %{secret: true})
  ...
  defp phase2_plan do
    assert {:ok, plan} =
             ProductionProfile.phase2_evidence_plan(:linear_cnb_shadow,
               linear_cnb_shadow_run_id: "bundle-command-shadow"
             )
    ...
    Enum.flat_map(runbook["entries"], fn entry ->
      Enum.map(entry["scenario_checklist"], fn scenario ->
        %{
          ...
          "collected_at" => "2026-06-26T04:45:00Z",
          ...
          "shadow" => %{
            ...
            "run_id" => get_in(entry, ["shadow_requirements", "run_id"]) || "bundle-command-shadow",
  ...
  defp non_claim_acknowledgements(runbook) do
    Enum.map(runbook["entries"], fn entry ->
      %{
        ...
        "acknowledged_at" => "2026-06-26T04:45:00Z"
      }
    end)
  end
