...
1. Read the {% if issue.entity_type == "bug" %}Bug{% else %}Story{% endif %} through `tracker.issue_snapshot` with comments included and `comment_limit: 100`. {% if issue.entity_type == "bug" %}Before any other action, inspect every existing `### Confusions` entry and verify whether each described problem still exists. Do not treat the mere presence of historical Confusions as a blocker. If any problem still exists, record the complete current Confusions evidence in the canonical workpad by calling `tracker.upsert_workpad` with `outcome: blocked_confusions`, then stop; that machine-readable outcome makes Maestro end the worker immediately and set `AI特殊工作流` to `AI异常`. Never use `blocked_confusions` for a resolved or historical-only Confusions entry. If every problem is gone, mark each corresponding Confusions entry resolved in the workpad (for example `- [x] ... — resolved: <evidence>`) and continue the normal workflow. Never use the deleted `AI是否遇到异常` field. If the Bug was previously completed and has been returned to `接受/处理`, treat it as a rework round: identify every human feedback comment whose id is not already recorded in a completed round, use those comment ids and timestamps as the incremental feedback boundary, and append a new `Rework Round N` entry to the same workpad. Never create a second workpad or silently treat a previously checked plan as proof that the new feedback is complete.{% endif %} Read or create exactly
...
3. {% if issue.entity_type == "bug" %}For a rework round, read the exact previously published `branch_name` and `published_head_sha` from the canonical workpad. Fetch the remote with `${SYMPHONY_WORKSPACE_AUTOMATION_DIR}/bin/repo fetch`, then use `repo_checkout` with `mode: create_or_switch`, that exact working branch name, and `base: origin/<working-branch>`; confirm the checked-out history contains the previous published SHA. Reuse the original working branch instead of creating a second branch. For an initial round, {% endif %}use {% if issue.branch_name %}`origin/{{ issue.branch_name }}` from the
...
8. After a successful push, update the canonical workpad and mirror with the
   ...
   These fields are handoff text only; do not create or update an MR. {% if issue.entity_type == "bug" %}For every completed rework round, record the handled human feedback comment ids, the reused branch name, the new published SHA, and the round validation result. Prepare one final full workpad body that preserves prior rounds and changes the final AI workflow checklist item to `- [x]`, with a note that `AI特殊工作流=AI已解决` was confirmed by typed-tool read-back. Do not write that final body with `tracker.upsert_workpad`; pass it to the completion tool so it cannot appear before field verification succeeds.{% endif %}
9. {% if issue.entity_type == "bug" %}Only after the code change is validated, committed, pushed, and recorded in the workpad, call `tracker.complete_ai_workflow` with `issue_id`, the canonical `workpad_id`, and the prepared final workpad `body`. The tool must change `AI特殊工作流` from `接受/处理` to `AI已解决`, read the Bug back, verify the resolved value, and only then update that same workpad with the final body. A stale read-back or final workpad update failure is not success. Do not change the Bug's TAPD status. After the tool succeeds, mirror the same final body in workspace-root `.symphony-tapd-workpad.md` and stop; Maestro then removes this task workspace but retains the shared Git object cache.{% else %}Move the Story to the configured `review` raw state through the typed tracker move tool. Stop immediately after the state update succeeds because review is a human wait state and is not active for this template.{% endif %}
