defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.OperatorCommands.ProductionProfileEvidenceHandoff do
  @moduledoc """
  Operator command for Coding PR Delivery Phase 2 evidence handoff packages.

  The command reads bounded metadata files for the Phase 2 plan, provider
  preflight report, evidence request, and completed evidence packets. It emits a
  single provider-owner handoff package without reading referenced evidence
  files, calling providers, mutating workflow state, approving production, or
  enabling gates.
  """
  ...
  @type handoff_result :: {:ok, map()} | {:error, map()}
  @type deps :: %{
          required(:read_file) => (String.t() -> {:ok, binary()} | {:error, term()}),
          required(:production_evidence_handoff) => (String.t() | map(), keyword() -> handoff_result())
        }
    ...
    case OptionParser.parse(argv, strict: @switches, aliases: [h: :help]) do
      {parsed_opts, [], []} ->
        if parsed_opts[:help] do
          ...
          render_handoff(parsed_opts, deps)
        end
  ...
  defp render_handoff(parsed_opts, deps) do
    with {:ok, input} <- handoff_input(parsed_opts, deps),
         {:ok, opts} <- handoff_opts(parsed_opts, deps),
         {:ok, handoff} <- deps.production_evidence_handoff.(input, opts) do
      {Jason.encode!(handoff, pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", 0}
    else
      {:error, {:usage, message}} ->
        {"", message <> "\n" <> usage(), @usage_error_exit_code}

      {:error, reason} ->
        {Jason.encode!(handoff_error(reason), pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", @validation_failed_exit_code}
    end
  end

  defp handoff_input(parsed_opts, deps) do
  ...
  defp handoff_opts(parsed_opts, deps) do
    opts =
      []
  ...
  defp handoff_error(reason) do
    %{
      "schema" => "coding_pr_delivery.production_evidence_handoff_result.v1",
      "status" => "invalid",
      "valid" => false,
  ...
  defp error_to_map(_error), do: %{"code" => "production_evidence_handoff_error", "message" => "Evidence handoff projection failed."}
  ...
  defp validate_deps(%{read_file: read_file, production_evidence_handoff: handoff})
       when is_function(read_file, 1) and is_function(handoff, 2),
       do: :ok
  ...
  defp usage do
    ...
    This command emits a bounded Phase 2 provider-evidence handoff package. It
