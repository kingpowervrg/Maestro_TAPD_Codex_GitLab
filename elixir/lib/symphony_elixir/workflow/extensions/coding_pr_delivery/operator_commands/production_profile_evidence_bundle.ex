defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.OperatorCommands.ProductionProfileEvidenceBundle do
  @moduledoc """
  Operator command for Coding PR Delivery Phase 2 evidence bundle readiness.

  The command reads bounded metadata files for the Phase 2 plan, provider
  preflight report, evidence request, and completed evidence packets. It does
  not read referenced evidence files, call providers, mutate workflow state,
  ...
  @command_id "symphony.workflow.extension.coding_pr_delivery.production_profile_evidence_bundle"
  ...
  @switches [
    ...
    evidence_request_file: :string,
    preflight_report_file: :string,
    evidence_packet_file: :string,
    plan_id: :string,
  ...
  @type bundle_result :: {:ok, map()} | {:error, map()}
  @type deps :: %{
          required(:read_file) => (String.t() -> {:ok, binary()} | {:error, term()}),
          required(:production_evidence_bundle) => (String.t() | map(), keyword() -> bundle_result())
        }
  ...
  def runtime_deps do
    %{
      ...
      production_evidence_bundle: fn input, opts -> ProductionProfile.production_evidence_bundle(input, opts) end
    }
  end
    ...
    case OptionParser.parse(argv, strict: @switches, aliases: [h: :help]) do
      {parsed_opts, [], []} ->
        if parsed_opts[:help] do
          ...
          render_bundle(parsed_opts, deps)
        end
  ...
  defp render_bundle(parsed_opts, deps) do
    with {:ok, input} <- bundle_input(parsed_opts, deps),
         {:ok, opts} <- bundle_opts(parsed_opts, deps),
         {:ok, bundle} <- deps.production_evidence_bundle.(input, opts) do
      {Jason.encode!(bundle, pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", 0}
    else
      {:error, {:usage, message}} ->
        {"", message <> "\n" <> usage(), @usage_error_exit_code}

      {:error, reason} ->
        {Jason.encode!(bundle_error(reason), pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", @validation_failed_exit_code}
    end
  end

  defp bundle_input(parsed_opts, deps) do
  ...
  defp bundle_opts(parsed_opts, deps) do
    opts =
      []
    ...
    with {:ok, opts} <- maybe_put_json_file(opts, parsed_opts, deps, :evidence_request_file, :evidence_request, "Evidence request"),
         {:ok, opts} <- maybe_put_json_file(opts, parsed_opts, deps, :preflight_report_file, :preflight_report, "Preflight report"),
         {:ok, packets} <- read_evidence_packet_files(parsed_opts, deps) do
      {:ok, Keyword.put(opts, :evidence_packets, packets)}
    end
  end

  defp maybe_put_json_file(opts, parsed_opts, deps, file_key, opt_key, label) do
    case Keyword.get(parsed_opts, file_key) do
      nil ->
        {:ok, opts}

      file_path ->
        with {:ok, packet} <- read_json_file(file_path, label, deps) do
          {:ok, Keyword.put(opts, opt_key, packet)}
        end
    end
  end

  defp read_evidence_packet_files(parsed_opts, deps) do
    parsed_opts
    |> Keyword.get_values(:evidence_packet_file)
    |> Enum.reduce_while({:ok, []}, fn file_path, {:ok, acc} ->
      case read_json_file(file_path, "Evidence packet", deps) do
        {:ok, packet} -> {:cont, {:ok, [packet | acc]}}
  ...
  defp bundle_error(reason) do
    %{
      "schema" => "coding_pr_delivery.production_evidence_bundle_result.v1",
      "status" => "invalid",
      "valid" => false,
  ...
  defp validate_deps(%{read_file: read_file, production_evidence_bundle: bundle})
       when is_function(read_file, 1) and is_function(bundle, 2),
       do: :ok
  ...
  defp usage do
    """
    Command arguments:
      [--plan <id>|--phase2-plan-file <file>] [--evidence-request-file <file>]
      [--preflight-report-file <file>] [--evidence-packet-file <file>]... [--json|--pretty]
    ...
    This command emits a bounded Phase 2 evidence bundle readiness packet. It
    does not read referenced evidence files, call providers, mutate workflow
