defmodule SymphonyElixir.Workflow.Extensions.CodingPrDelivery.OperatorCommands.ProductionProfileEvidenceRequest do
  @moduledoc """
  Operator command for Coding PR Delivery Phase 2 evidence requests.

  The command emits a bounded provider-owner handoff packet naming required
  credentials, auth checks, concrete targets, preflight probes, and evidence
  files. It does not read referenced evidence files, call providers, mutate
  ...
  @type request_result :: {:ok, map()} | {:error, map()}
  @type deps :: %{
          required(:read_file) => (String.t() -> {:ok, binary()} | {:error, term()}),
          required(:production_evidence_request) => (String.t() | map(), keyword() -> request_result())
        }
  ...
  def runtime_deps do
    %{
      ...
      production_evidence_request: fn input, opts -> ProductionProfile.production_evidence_request(input, opts) end
    }
  end
    ...
    case OptionParser.parse(argv, strict: @switches, aliases: [h: :help]) do
      {parsed_opts, [], []} ->
        if parsed_opts[:help] do
          ...
          render_request(parsed_opts, deps)
        end
  ...
  defp render_request(parsed_opts, deps) do
    with {:ok, input} <- request_input(parsed_opts, deps),
         {:ok, request} <- deps.production_evidence_request.(input, request_opts(parsed_opts)) do
      {Jason.encode!(request, pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", 0}
    else
      {:error, {:usage, message}} ->
        {"", message <> "\n" <> usage(), @usage_error_exit_code}

      {:error, reason} ->
        {Jason.encode!(request_error(reason), pretty: Keyword.get(parsed_opts, :pretty, false)) <> "\n", "", @validation_failed_exit_code}
    end
  end

  defp request_input(parsed_opts, deps) do
    case Keyword.get(parsed_opts, :phase2_plan_file) do
      nil -> {:ok, parsed_opts |> Keyword.get(:plan, "tiered_reference") |> String.trim()}
      file_path -> read_json_file(file_path, deps)
    end
  end

  defp request_opts(parsed_opts) do
    []
  ...
  defp read_json_file(file_path, deps) do
    with {:ok, contents} <- deps.read_file.(file_path),
      ...
      {:error, _reason} ->
        {:error, {:usage, "Phase 2 plan file could not be read or parsed."}}

      false ->
        {:error, {:usage, "Phase 2 plan file must contain an object."}}
    end
  end
  ...
  defp request_error(reason) do
    %{
      "schema" => "coding_pr_delivery.production_evidence_request_result.v1",
      "status" => "invalid",
      "valid" => false,
  ...
  defp error_to_map(_error), do: %{"code" => "production_evidence_request_error", "message" => "Evidence request projection failed."}
  ...
  defp validate_deps(%{read_file: read_file, production_evidence_request: request})
       when is_function(read_file, 1) and is_function(request, 2),
       do: :ok
  ...
  defp usage do
    """
    Command arguments:
      [--plan <id>|--phase2-plan-file <file>] [--json|--pretty]
    ...
    This command emits a bounded Phase 2 provider-evidence request packet. It
